...
class BaseRemoteCLIPSeg(nn.Module):  
    ...
    def __init__(  
        ...
        # 初始化主干  
        self.visual_encoder = self._init_and_patch_encoder(
    ...
    def _init_and_patch_encoder(self, model_name, in_channels, ckpt_path, freeze_clip):  
        ...
        enc = model.visual # type: nn.Module  
        enc.eval()  
        ...
        with torch.no_grad():  
            ...
            new_conv1.weight[:num_copy, 3:, :, :] = orig[:num_copy, :1, :, :]  # 假设4通道or更多时平铺最后一通道  
        enc.conv1 = new_conv1  
    ...
    def encoder(self):  
        return (  
            self.visual_encoder.module  
            if hasattr(self.visual_encoder, "module")  
    ...
    def extract_encoder_features(self, x):  
        ...
        features = []  
        enc = self.encoder  
        # x = x.to(self.main_device)   # DataParallel环境下不能暴力指定
        with torch.no_grad():  
            enc.eval()  
            x = enc.conv1(x)  
            x = enc.bn1(x)  
    ...
    def to(self, device):  
        super().to(device)  
        if hasattr(self.visual_encoder, "to"):  
